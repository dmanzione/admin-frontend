...
const AccountInfo = () => {
    ...
    useEffect(() => {
        const fetchAccount = async () => {
                 ...
                 axios.get(`http://localhost:8080/accounts-api/accounts/${accountId}`).then((response) => {
        ...
        const fetchFinancialProducts = async () => {
                   axios.get(`http://localhost:8080/accounts-api/accounts/${account!.pk}/products`).then(resp => {
                    ...
                    // eslint-disable-next-line no-lone-blocks
                    {
                        financialProducts.map((financialProduct: FinancialProduct) => {
                            if(financialProduct.name?.toLowerCase() === 'loan') {
                             ...
                             if(financialProduct.name?.toLowerCase() === 'card') {
                                setCards(cards => [...cards, financialProduct as Card]);
                            } else
                             if(financialProduct.name?.toLowerCase() === 'reward') {
                                setRewards(rewards => [...rewards, financialProduct as Reward]);
                            }
                    }
                            );
                    }
    ...
    <div>
        ...
        <Table>
            <tbody>
                ...
                {loans.length > 0 && ( <tr> <td>Loans:</td><td> {loans.map((loan:Loan) => { 
                ...
                {cards.length > 0 && ( <tr> <td>Cards:</td><td> {cards.map((card:Card) => {
                ...
                {rewards.length > 0 && ( <tr> <td>Rewards:</td><td> {rewards.map((reward:Reward) => {
